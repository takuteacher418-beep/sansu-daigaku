(() => {
  ...
  function renderStudentHome() {
    ...
    $("#recommendationBox").innerHTML = `<strong>${recommendation.unit}｜${recommendation.title}</strong><p>${student.profile.visual ? "図を使って" : "自分の言葉で"}考えられます。</p>`;
  ...
  function renderProblem() {
    ...
    $("#problemProfessor").textContent = activeProblem.professor;
  ...
  $("#submitAnswerBtn").addEventListener("click", () => {
    ...
    $("#feedbackBox").innerHTML = `
      ...
      <strong>${score >= 60 ? "教授は納得した！" : "もう少しで伝わりそう！"}</strong>
